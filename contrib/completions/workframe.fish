...
set -l commands init setup clone new resume current run dashboard list open path archive restore remove repos worktrees status doctor config sync clean migrate update completion
...
for command in workframe wf
  ...
  complete -c $command -n "__fish_seen_subcommand_from migrate" -l yes
end
