...
pid, fd = pty.fork()
if pid == 0:
    os.execv(os.environ["WORKSPACES"], [os.environ["WORKSPACES"], "setup"])
...
@test "setup wizard accepts defaults and keeps stdout usable" {
  ...
  run env HOME="$home" XDG_CONFIG_HOME="$XDG_CONFIG_HOME" WORKSPACES="$WORKSPACES" \
    python3 "$PTY_RUNNER" '\n\n'

  [ "$status" -eq 0 ]
  [[ "$output" == *'? Collection root ['*'~/workspaces]:'* ]]
  [[ "$output" == *'Repositories'*"$home/workspaces/repos"* ]]
  [[ "$output" == *'? Set up this collection? [Y/n]:'* ]]
  [ -d "$home/workspaces/repos" ]
  [ -d "$home/workspaces/worktrees" ]
}
